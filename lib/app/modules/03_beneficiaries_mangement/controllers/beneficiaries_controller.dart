...
class BeneficiariesController extends GetxController {
  ...
  void fetchAllBeneficiaries() async {
    try {
      ...
      Get.defaultDialog(
          title: "خطأ", middleText: "فشل جلب قائمة المستفيدين.");
    } finally {
      isLoading(false);
    }
  }
  ...
  void openAddEditDialog({BeneficiaryModel? beneficiary}) {
    ...
    } else { // تفريغ الحقول في حالة الإضافة
    ...
    Get.dialog(
        AlertDialog(
            title: Text(
                beneficiary == null ? 'إضافة مستفيد جديد' : 'تعديل مستفيد'),
            content: Form(
                key: formKey,
                child: Column(
                ...
                TextFormField(controller: typeController,
                ...
                TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'الرقم التعريفي (إن وجد)'),
                ),
                  ],
                ),
            ),
    ...
    ElevatedButton(
      onPressed: () => saveBeneficiary(beneficiary), child: const Text('حفظ'),),
            ],
        ),
    );
  }
  ...
  void saveBeneficiary(BeneficiaryModel? beneficiary) async {
    ...
    Get.defaultDialog(
    title: "خطأ", middleText: "فشل الحفظ: قد يكون الاسم مكرراً.");
    }
  }
}
...
void deleteBeneficiary(int id) {
  Get.defaultDialog(
      title: "تأكيد الحذف",
      middleText:
      "هل أنت متأكد منحذف هذا المستفيد؟",
      textConfirm: "حذف",
      textCancel: "إلغاء",
      onConfirm: () async {
        Get.back();
        ...
        // تحديث قائمةالمستفيدين في شاشة أوامر الصرف أيضاً
          ...
          Get.defaultDialog(
              title: "خطأ",
              middleText: "فشل الحذف: قد يكون هذا المستفيد مستخدماً في أحد أوامر الصرف.");
        }
      },
  );
}
